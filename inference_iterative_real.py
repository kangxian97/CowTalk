...
For each sample (output_dir / sample_name/):
- input_volume.nii.gz  - input segmentation volume
- gt_volume.nii.gz     - ground truth segmentation volume
- pass4_volume.nii.gz  - predicted volume after iterative correction
- CNN uses 1-channel input only (segmentation volume; no medical image).
"""
...
import models
from dataset import _normalize_coordinates, _sample_points_from_volume
...
def _get_initial_error_classes_from_instruction(instruction_file: Path) -> set:
    """Parse instruction JSON and return set of class IDs (1-13) that have error text."""
    out = set()
    if not instruction_file.exists():
        return out
    try:
        ...
        for label_id in range(1, 14):
            ...
            if not isinstance(info, dict):
                continue
            text_str = info.get("long", info.get("short", ""))
            if not (text_str and text_str.strip()):
                continue
            if "n/a" in text_str.lower() or "no error" in text_str.lower():
                continue
            out.add(label_id)
    except Exception:
        pass
...
def load_one_sample(
    ...
    input_points: int,
    query_points: int,
    input_fg_ratio: float,
    input_random_ratio: float,
    query_fg_ratio: float,
    query_random_ratio: float,
    medical_image: np.ndarray = None,
) -> dict:
    """
    Load one sample for inference directly from NPZ and instruction, without using the dataset.
    Mirrors the inference_mode path of dataset.CowPointCloudDataset.__getitem__.
    """
    from pointcloud import load_pointcloud, scatter_labels

    cloud = load_pointcloud(npz_path)
    gt_cropped = scatter_labels(cloud["volume_shape"], cloud["fg_coords"], cloud["fg_gt_labels"])
    modified_cropped = scatter_labels(cloud["volume_shape"], cloud["fg_coords"], cloud["fg_error_labels"])
    modified_resized = cloud["modified_resized"]

    volume_shape = np.array(gt_cropped.shape, dtype=np.float32)
    bbox = np.stack([np.zeros(3, dtype=np.float32), volume_shape - 1], axis=0)

    initial_error_classes = _get_initial_error_classes_from_instruction(instruction_file)
    input_error_classes = set()
    for c in initial_error_classes:
        if rng.random() >= 0:
            input_error_classes.add(c)
    remaining_error_classes = set()
    for c in input_error_classes:
        if rng.random() >= 0.50:
            remaining_error_classes.add(c)
    ...
    if remaining_error_classes:
        for c in remaining_error_classes:
            current_volume[gt_cropped == c] = 0
            current_volume[modified_cropped == c] = c
    input_volume = current_volume.copy()
    for c in manipulated_error_classes:
        input_volume[current_volume == c] = 0
        input_volume[modified_cropped == c] = c

    input_coords, input_labels = _sample_points_from_volume(
        input_volume, input_points, rng, fg_ratio=input_fg_ratio, random_ratio=input_random_ratio
    )
    ...
    query_coords = np.stack(
        [z_coords.flatten(), y_coords.flatten(), x_coords.flatten()], axis=1
    )
    query_coords_original = query_coords.copy()

    query_labels = current_volume[
        query_coords[:, 0].astype(int),
        query_coords[:, 1].astype(int),
        query_coords[:, 2].astype(int),
    ]
    query_labels_input = input_volume[
        query_coords[:, 0].astype(int),
        query_coords[:, 1].astype(int),
        query_coords[:, 2].astype(int),
    ]

    input_coords_norm = _normalize_coordinates(input_coords, bbox)
    query_coords_norm = _normalize_coordinates(query_coords, bbox)
    native_shape = volume_shape
    query_coords_float = query_coords.astype(np.float32, copy=False)
    query_coords_resized_norm = (query_coords_float / np.maximum(native_shape - 1.0, 1.0)) * 2.0 - 1.0
    query_coords_resized = np.flip(query_coords_resized_norm, axis=-1).astype(np.float32, copy=False)

    input_labels_feature = input_labels.astype(np.float32).reshape(-1, 1)
    input_features = np.concatenate([input_coords_norm, input_labels_feature], axis=1)

    from pointcloud import stack_cnn_input
    concatenated_volume = stack_cnn_input(modified_resized, medical_image)
    ...
    return {
        ...
        "query_coords_original": torch.from_numpy(np.ascontiguousarray(query_coords_original, dtype=np.float32)),
        "query_coords_resized": torch.from_numpy(np.ascontiguousarray(query_coords_resized, dtype=np.float32)),
        "native_volume_shape": torch.from_numpy(volume_shape.astype(np.int64)),
        "input_volume": torch.from_numpy(input_volume.astype(np.int64)),
        "gt_volume": torch.from_numpy(gt_cropped.astype(np.int64)),
        "input_error_classes": torch.from_numpy(np.array(sorted(input_error_classes), dtype=np.int64)),
        "modified_resized": torch.from_numpy(concatenated_volume),
    }
...
def main():
    ...
    # Build sample list from instruction folder: for each instruction JSON, get basename and npz path
    print("Building sample list from instruction folder (no dataset)...", flush=True)
    ...
    sample_list = []
    for jpath in sorted(instructions_dir.glob("*.json")):
        base_name = jpath.stem
        npz_path = pointcloud_data_root / f"{base_name}.npz"
        if npz_path.exists():
            sample_list.append((base_name, npz_path))
        else:
            print(f"  Warning: no NPZ for instruction '{base_name}.json' at {npz_path}, skipping.", flush=True)
    if not sample_list:
        print("ERROR: No (instruction JSON, NPZ) pairs found. Exiting.", flush=True)
        return
    print(f"Found {len(sample_list)} samples (instruction JSON + matching NPZ).", flush=True)

    # Build model (get point_feature_dim from first sample)
    print("Building model...", flush=True)
    rng_for_init = np.random.Generator(np.random.PCG64(args.seed))
    base_name_0, npz_path_0 = sample_list[0]
    instruction_file_0 = instructions_dir / f"{base_name_0}.json"
    from pointcloud import load_medical_image
    medical_0 = load_medical_image(medical_images_dir, f"{base_name_0}.npz") if use_image else None
    sample_0 = load_one_sample(
        npz_path_0,
        base_name_0,
        instruction_file_0,
        rng_for_init,
        ...
        args.query_points,
        args.input_fg_ratio,
        args.input_random_ratio,
        args.query_fg_ratio,
        args.query_random_ratio,
        medical_image=medical_0,
    )
    point_feature_dim = sample_0["points"].shape[-1]
    ...
    # Process each sample (load directly from NPZ + instruction, no dataset)
    print(f"\nRunning iterative inference on {len(sample_list)} samples...", flush=True)
    print(f"Results will be saved to: {output_dir}", flush=True)

    rng = np.random.Generator(np.random.PCG64(args.seed))

    for idx in range(len(sample_list)):
        base_name, npz_path = sample_list[idx]
        instruction_file = instructions_dir / f"{base_name}.json"
        sample_name = base_name

        print(f"\n{'='*60}", flush=True)
        print(f"Processing sample {idx + 1}/{len(sample_list)}: {sample_name}", flush=True)
        print(f"{'='*60}", flush=True)

        medical_image = load_medical_image(medical_images_dir, f"{base_name}.npz") if use_image else None
        ...
        # Save input and ground truth volumes
        save_volume_as_nifti(input_volume, sample_output_dir / "input_volume.nii.gz")
        save_volume_as_nifti(gt_volume, sample_output_dir / "gt_volume.nii.gz")

        # Metrics: input (error shape) vs ground truth
        input_dice = compute_macro_avg_dice(input_volume, gt_volume, num_classes)
        input_f1 = compute_macro_avg_f1(input_volume, gt_volume, num_classes)
        input_chamfer = compute_macro_avg_chamfer(input_volume, gt_volume, num_classes)
        input_nsd = compute_macro_avg_surface_dice(input_volume, gt_volume, num_classes)
        print(f"  Input vs GT  - Dice: {input_dice:.4f}, F1: {input_f1:.4f}, Chamfer: {input_chamfer:.4f}, NSD: {input_nsd:.4f}", flush=True)
        ...
        pred_path = sample_output_dir / "pass4_volume.nii.gz"
        save_volume_as_nifti(current_volume, pred_path)

        # Metrics: predicted shape vs ground truth
        pred_dice = compute_macro_avg_dice(current_volume, gt_volume, num_classes)
        pred_f1 = compute_macro_avg_f1(current_volume, gt_volume, num_classes)
        pred_chamfer = compute_macro_avg_chamfer(current_volume, gt_volume, num_classes)
        pred_nsd = compute_macro_avg_surface_dice(current_volume, gt_volume, num_classes)
        print(f"  Pred vs GT   - Dice: {pred_dice:.4f}, F1: {pred_f1:.4f}, Chamfer: {pred_chamfer:.4f}, NSD: {pred_nsd:.4f}", flush=True)
        print(f"  Saved input_volume.nii.gz, gt_volume.nii.gz, pass4_volume.nii.gz", flush=True)
    
    print(f"\n{'='*60}", flush=True)
    print(f"All samples processed. Predictions saved to: {output_dir}", flush=True)
    print(f"{'='*60}", flush=True)
